...
(defn memorize-walk [visited pos]
  "Put current x/y position into visited set"
  (conj visited pos))
...
(defn split-directions [directions]
  (loop [put-into-robo true robo-dirs (vector) santa-dirs (vector) dirs directions]
    (cond
      (empty? dirs) [robo-dirs santa-dirs]
      put-into-robo (recur false (conj robo-dirs (first dirs)) santa-dirs (rest dirs))
      :else (recur true robo-dirs (conj santa-dirs (first dirs)) (rest dirs)))))

(defn next-count [count visited pos]
  (if (contains? visited pos)
    count
    (inc count)))

;; arbitrary walking
(defn walk [directions visited]
  (loop [dirs directions
         _visited visited
         pos [0 0]]
    (let [direction (first dirs)
          new-pos (next-position direction pos)
          new-visited (memorize-walk _visited new-pos)]
      (if (empty? dirs)
        _visited
        (recur (rest dirs) new-visited new-pos)))))

;; santa walking
(defn walk-santa [directions]
  (count (walk (seq directions) #{[0 0]})))

;; santa and robo walking
(defn walk-robo-and-santa [directions]
  (let [[rdirs sdirs] (split-directions directions)
        visited (walk rdirs #{[0 0]})]
    (count (walk sdirs visited))))

;; (walk-santa input)
;; --> 2565

;;(walk-robo-and-santa input)
;; --> 2639
